...
class ClientAdmin(PublicSchemaOnlyAdminMixin, admin.ModelAdmin): # 👈 حقن درع الحظر المركزي هنا لحماية الشركات
    ...
    def enter_tenant_dashboard(self, obj):
        try:
            ...
            # الاستغناء الذكي عن رقم البورت الثابت ليتناسب مع النطاق الفعلي لايف ومحلياً بمرونة
...
class BlindBiddingRequestAdmin(admin.ModelAdmin):
    ...
    # 🚀 🚀 تصحيح وهندسة الـ Actions لفض النزاعات محاسبياً (Full Atomic Integration):
    ...
    def resolve_dispute_refund(self, request, queryset):
        ...
        # فرز الطلبات المتنازع عليها فقط واللف عليها لاستدعاء اللوجيك المالي الذري
        disputed_bids = queryset.filter(status='disputed')
        
        for bid in disputed_bids:
            try:
                with transaction.atomic():
                    bid.trigger_refund_to_buyer() # تشغيل المعالج المالي الحقيقي لتحديث الخزائن والـ Ledger
                    success_count += 1
    ...
    def resolve_dispute_release(self, request, queryset):
        ...
        for bid in disputed_bids:
            try:
                with transaction.atomic():
                    # محاكاة تخطي الشحن استثنائياً بقرار من الإدارة لفض النزاع
                    bid.status = 'shipped' 
                    bid.trigger_release_to_seller() # تشغيل المعالج المالي الحقيقي لخصم الضمان وضخ صافي الأرباح للتاجر
                    success_count += 1
            except ValidationError as ve:
                error_count += 1
...
class EscrowLedgerAdmin(PublicSchemaOnlyAdminMixin, admin.ModelAdmin): 
    """🚀 مراقبة وتتبع صارم لكل قرش يتحرك داخل منصة Mouss Tec للإدارة العليا فقط (غير قابلة للتلاعب)"""
    ...
    def fraud_risk_badge(self, obj):
        # رادار أمني ذكي لرصد عمليات السحب الكبيرة ومكافحة غسيل الأموال أو الاحتيال بالسوق
